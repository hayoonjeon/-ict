...
/*
 	...
 	1. Member 내부클래스 :  내부클래스는 외부클래스의 멤버필드와 멤버메서드를 마음대로 사용 가능 (상속과 비슷)
 	                     단, 상속 private 접근 불가,  내부클래스는 private 접근 가능
 	                     외부클래스를 통해서남 내부클래스를 생성할 수 있다.
 	                     (내부클래스 별도로 생성 불가능)
 	   외부클래스{
 	      멤버필드;
 	      멤버메서드();
 	  ...
 	  2. Local 내부 클래스 : 외부클래스의 메서드 안에 내부클래스가 존재
 	                      외부클래스에서는 지역변수처럼 사용한다.
 	                      내부클래스를 가진 외부클래스의 메서드가 실행되어야 내부클래스를 생성할 수 있다. 
 	                      지역변수 처럼 사용되기 때문에 내부클래스를 가진 외부클래스의 메서드가 종료되면 
 	                      내부 클래스도 사라진다.
 	 외부클래스{
 	      멤버필드;
 	      멤버메서드();
 	...
 	3. static 내부 클래스 : Member 내부클래스와 같은 구조를 가지고 있음
                         ...
                         내부클래스 자체를 static 으로 만들어야 한다.
                         접근법은 static 접근법과 같다 (내부클래스이름.변수이름)  
      외부클래스{
 	      멤버필드;
 	      멤버메서드();
