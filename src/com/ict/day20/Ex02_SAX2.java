...
public class Ex02_SAX2 {
	
	 static FileWriter fw = null;
	 static BufferedWriter bw = null;
	 
	public static void main(String[] args) {
		Path path = Paths.get("D:", "Hayoon","util", "0826-3.txt");
			...
			DefaultHandler handler = new DefaultHandler() {
				...
				public void startElement(String uri, String localName, String qName, Attributes attributes)
					...
					if (qName.equalsIgnoreCase("weather")) {
						String y = attributes.getValue("year");
						String m = attributes.getValue("month");
						String d = attributes.getValue("day");
						String h = attributes.getValue("hour");
						sb2.append("날짜 : ").append(y+ ".").append(m+".").append(d+".").append(h+"시\n");
					}
					
					if(qName.equalsIgnoreCase("local")) {
						isLocal = true;
						txt.setLength(0); // 텍스트 부분 초기화
				...
				public void endElement(String uri, String localName, String qName) throws SAXException {
					if(qName.equalsIgnoreCase("local")) {
						String cityName = txt.toString().trim(); // 도시 이름 완성
						sb2.append(cityName+"\t").append(attr1+"\t").append(attr2+"\n");
						isLocal = false;
					}
				}
